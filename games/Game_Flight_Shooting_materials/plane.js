    ...
    document.onmousedown = function(ev) {
        ...
        if (ev.which == 1) {
            ...
            function bulletshoot() {
                var bullet = new Bullet(that.plane.offsetLeft + that.w / 2 - 3, that.plane.offsetTop - 14, 6, 14, 'Game_Flight_Shooting_materials/img/bullet.png');
            }
        }
        return false;
    }
...
var myplane = new Myplane(66, 80, 'Game_Flight_Shooting_materials/img/myplane.png', 'Game_Flight_Shooting_materials/img/boom_2.jpg');

for (var i = 0; i < getrandom(1, 2); i++) { //随机创建各类飞机
    var timer = setInterval(function() {
        var num = getrandom(1, 20); //1-15 小飞机  16-20 中飞机  20打飞机
        if (num >= 1 && num < 15) {
            var enemy = new Enemyplane(getrandom(0, planeBox.offsetWidth - 34), -24, 34, 24, 'Game_Flight_Shooting_materials/img/smallplane.png', 'Game_Flight_Shooting_materials/img/boom_2.jpg', getrandom(3, 5), 1, 1);
        } else if (num >= 15 && num < 20) {
            var enemy = new Enemyplane(getrandom(0, planeBox.offsetWidth - 46), -60, 46, 60, 'Game_Flight_Shooting_materials/img/smallplane.png', 'Game_Flight_Shooting_materials/img/boom_2.jpg', getrandom(2, 4), 3, 5);
        } else if (num == 20) {
            var enemy = new Enemyplane(getrandom(0, planeBox.offsetWidth - 110), -164, 110, 164, 'Game_Flight_Shooting_materials/img/smallplane.png', 'Game_Flight_Shooting_materials/img/boom_2.jpg', getrandom(1, 2), 10, 10);
        }
    }, 1000)
}
